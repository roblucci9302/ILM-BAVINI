...
const AgentIcon = memo(({ agent }: { agent: AgentType }) => {
  ...
  return <div className={classNames(icons[agent] || 'i-ph:robot', 'text-lg')} />;
});
...
const ActionTypeBadge = memo(({ type }: { type: ProposedAction['type'] }) => {
  const colors: Record<ProposedAction['type'], string> = {
  ...
  return (
    <span className={classNames('px-2 py-0.5 text-xs rounded border', colors[type])}>
      {labels[type]}
    </span>
  );
});
 ...
 */
const CodePreview = memo(({ content, language, maxLines = 20 }: { content: string; language?: string; maxLines?: number }) => {
        ...
        <code className="text-bolt-elements-textPrimary">
          {displayLines.map((line, i) => (
            <div key={i} className="flex">
              <span className="select-none text-bolt-elements-textTertiary w-8 text-right pr-3">
                {i + 1}
              </span>
              <span>{line}</span>
            </div>
    ...
    <div className="mt-2 rounded-lg bg-bolt-elements-background-depth-3 border border-bolt-elements-borderColor overflow-hidden">
      ...
      <div className="p-3 text-xs max-h-64 overflow-auto">
        ...
        <pre className="bg-red-500/10 p-2 rounded mb-3 overflow-x-auto">
          <code className="text-red-300">{oldContent.slice(0, 500)}{oldContent.length > 500 ? '...' : ''}</code>
        </pre>
        <div className="mb-2">
        ...
        <pre className="bg-green-500/10 p-2 rounded overflow-x-auto">
          <code className="text-green-300">{newContent.slice(0, 500)}{newContent.length > 500 ? '...' : ''}</code>
        </pre>
      </div>
    </div>
  );
  ...
  const renderDetails = () => {
    ...
    switch (action.type) {
      case 'file_create':
        ...
        return (
          <CodePreview
            ...
            language={createDetails.language}
          />
        );
      ...
      case 'file_modify':
        ...
        return (
          <DiffPreview
            oldContent={modifyDetails.oldContent}
            newContent={modifyDetails.newContent}
          />
        );
      ...
      case 'shell_command':
        ...
        return (
          <div className="mt-2 p-3 rounded-lg bg-bolt-elements-background-depth-3 border border-bolt-elements-borderColor">
            ...
            <code className="text-sm text-bolt-elements-textPrimary font-mono">
              $ {shellDetails.command}
            </code>
            <div className="mt-2 text-xs text-bolt-elements-textTertiary">
              {shellDetails.commandCheck.message}
            </div>
          </div>
        );
        ...
        <button
          ...
          className={classNames(
            ...
            isSelected
              ...
              : 'border-bolt-elements-borderColor hover:border-accent-400'
          )}
        >
