...
// dark mode: aurora vibrant (teal, violet, rose, amber)
const DARK_COLORS = ['#14B8A6', '#8B5CF6', '#EC4899', '#F59E0B'];

// light mode: pastel aurora (softer versions)
const LIGHT_COLORS = ['#99F6E4', '#C4B5FD', '#FBCFE8', '#FDE68A'];
...
export default function ColorBends({
  ...
  // Track if we're using the fallback resize listener to ensure proper cleanup
  const usingFallbackResizeRef = useRef<boolean>(false);
  ...
  // use theme-based colors if no custom colors provided
  ...
  useEffect(() => {
    ...
    if ('ResizeObserver' in window) {
      ...
      usingFallbackResizeRef.current = false;
    } else {
      // Fallback for browsers without ResizeObserver
      (window as Window).addEventListener('resize', handleResize);
      usingFallbackResizeRef.current = true;
    }
    ...
    return () => {
      ...
      // Always clean up fallback listener if it was used
      if (usingFallbackResizeRef.current && resizeHandlerRef.current) {
        (window as Window).removeEventListener('resize', resizeHandlerRef.current);
        usingFallbackResizeRef.current = false;
      }
