    ...
    <AnimatePresence>
      ...
      >
        ...
        <div className="flex flex-col min-w-0">
          <span className="text-xs font-medium text-accent-400 truncate">
            {executingAgent}
          </span>
          {currentAction && (
...
export const WorkbenchAgentStatus = memo(({ className }: WorkbenchAgentStatusProps) => {
  ...
  return (
    ...
    >
      ...
      {activeCount > 0 && (
        <span className="text-accent-400 font-medium">{activeCount}</span>
      )}
    </div>
  );
});
...
export const EditorAgentOverlay = memo(({ filePath, className }: EditorAgentOverlayProps) => {
  ...
  const executingAgent = activeAgents.find(
    (agent) => agentStatuses[agent] === 'executing'
  );
...
export const TerminalAgentIndicator = memo(({ className }: TerminalAgentIndicatorProps) => {
  ...
  const isShellAction = currentAction?.type === 'shell' ||
    currentAction?.type === 'npm_install' ||
    currentAction?.type === 'npm_run';
