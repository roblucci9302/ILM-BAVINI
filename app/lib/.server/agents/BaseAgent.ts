...
export abstract class BaseAgent<TResponse> {
  ...
  protected validateAction(requiredCapability: AgentCapability): void {
    if (this.isRestricted(requiredCapability)) {
      throw new AgentRestrictionError(
        `Action "${requiredCapability}" is restricted in ${this.config.mode} mode`
      );
    }

    if (!this.hasCapability(requiredCapability)) {
      throw new AgentCapabilityError(
        `Agent does not have capability "${requiredCapability}"`
      );
    }
  }
  ...
  protected async analyzeCode(content: string): Promise<CodeAnalysis> {
    ...
    const hasTests = /\.(test|spec)\.(ts|tsx|js|jsx)$/.test(content) || content.includes('describe(') || content.includes('it(');
    ...
    for (const file of files) {
      if (file.path.includes('package.json')) {
        try {
          ...
          if (deps['remix'] || deps['@remix-run/react']) stack.add('Remix');
