...
export class AutoFixProcessor {
  ...
  async *processStream(
    ...
    this.stats.averageTimePerBlock =
      this.stats.blocksProcessed > 0
        ? this.stats.totalProcessingTime / this.stats.blocksProcessed
        : 0;
  ...
  private async applyFixes(code: string, context: FixContext): Promise<FixResult> {
    ...
    for (const rule of sortedRules) {
      if (fixCount >= this.options.maxFixesPerBlock) {
        allWarnings.push(
          `Maximum de ${this.options.maxFixesPerBlock} corrections atteint pour ce bloc`
        );
        break;
      }
  ...
  private getActiveRules(): FixRule[] {
    return this.rules.filter((rule) => {
      ...
      if (
        this.options.enabledRules.length > 0 &&
  ...
  private updateStats(category: FixCategory, count: number): void {
    this.stats.fixesByCategory[category] =
      (this.stats.fixesByCategory[category] || 0) + count;
  }
