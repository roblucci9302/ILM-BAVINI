...
export class UXPatternsEvaluator {
  ...
  private analyzeLoadingStates(
    ...
    const hasAsyncOperations =
      /useQuery|useMutation|fetch\(|axios|async\s+function|await\s+/.test(content);
   ...
   */
  private analyzeErrorStates(
    content: string,
    files: ExtractedFile[]
  ): { count: number; issues: QualityIssue[] } {
    const issues: QualityIssue[] = [];
