...
export class SecurityFixer implements FixRule {
  ...
  private fixDangerousHTML(code: string): {
    ...
    for (const match of matches) {
      ...
      if (/^["'`]/.test(htmlSource)) {
        warnings.push(
          `dangerouslySetInnerHTML utilisé avec une valeur littérale - vérifiez que le contenu est sûr`
        );
        continue;
      }
  ...
  private checkCodeInjection(code: string): {
    ...
    for (const { pattern, name } of injectionPatterns) {
      ...
      if (match) {
        ...
        warnings.push(
          `⚠️ ${name} détecté - cela peut permettre l'exécution de code malveillant`
        );
      }
    }
  ...
  private fixHardcodedSecrets(code: string): {
    ...
    for (const { pattern, envName, description } of secretPatterns) {
      ...
      while ((match = pattern.exec(fixedCode)) !== null) {
        ...
        warnings.push(
          `⚠️ N'oubliez pas d'ajouter ${envName} dans votre fichier .env`
        );
      }
    }
      ...
      if (!/rel\s*=/.test(attrs)) {
        ...
        newAttrs = newAttrs.replace(
          ...
          (_relMatch: string, relValue: string) => {
            let newRel = relValue;
            if (!relValue.includes('noopener')) newRel += ' noopener';
            if (!relValue.includes('noreferrer')) newRel += ' noreferrer';
