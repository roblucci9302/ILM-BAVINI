 ...
 */
export function createFixers(options: {
  imports?: boolean;
  typescript?: boolean;
  ...
  if (imports) fixers.push(new ImportFixer());
  if (typescript) fixers.push(new TypeScriptFixer());
  if (accessibility) fixers.push(new AccessibilityFixer());
  if (security) fixers.push(new SecurityFixer());
