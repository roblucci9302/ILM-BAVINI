...
const DEFAULT_OPTIONS: Required<Omit<AuditLoggerOptions, 'persistFn'>> & { persistFn?: (entry: AuditEntry) => Promise<void> } = {
  maxEntries: 10000,
  sessionId: 'default',
  persistFn: undefined,
};
...
export class AuditLogger {
  private options: Required<Omit<AuditLoggerOptions, 'persistFn'>> & { persistFn?: (entry: AuditEntry) => Promise<void> };
  ...
  private exportAsCSV(entries: AuditEntry[]): string {
    const headers = ['ID', 'Timestamp', 'Session', 'Type', 'Target', 'Name', 'Success', 'Duration (ms)', 'Risk Level', 'Warnings'];
