...
describe('ReviewManager', () => {
  ...
  describe('submitDecision', () => {
    ...
    it('should throw for non-existent review', async () => {
      await expect(
        manager.submitDecision('non_existent_id', 'approve')
      ).rejects.toThrow();
    });
    ...
    it('should throw for already processed review', async () => {
      ...
      await expect(
        manager.submitDecision(request.id, 'approve')
      ).rejects.toThrow();
    });
  });
...
describe('REVIEW_MESSAGE_TEMPLATES', () => {
  it('should generate schema creation message', () => {
    const message = REVIEW_MESSAGE_TEMPLATES.schemaCreation(
      ['users', 'posts'],
      'CREATE TABLE users...'
    );
  ...
  it('should generate destructive change message', () => {
    const message = REVIEW_MESSAGE_TEMPLATES.destructiveChange(
      'DROP TABLE',
      ['users', 'posts']
    );
  ...
  it('should generate migration review message', () => {
    const message = REVIEW_MESSAGE_TEMPLATES.migrationReview(
      { name: 'add_users', up: 'CREATE TABLE users...' },
      true,
      ['Warning 1']
    );
  ...
  it('should generate RLS review message', () => {
    const message = REVIEW_MESSAGE_TEMPLATES.rlsReview([
      { name: 'users_select', table: 'users', action: 'SELECT' },
    ]);
