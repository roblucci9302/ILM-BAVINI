...
export class RollbackManager {
   ...
   * Compare deux tables
   */
  private diffTables(current: Table, target: Table): {
    hasChanges: boolean;
    ...
    const hasChanges = addedColumns.length > 0 ||
  ...
  private generateRollbackSQL(diff: SchemaDiff): string {
    ...
    for (const tableDiff of diff.modifiedTables) {
      ...
      for (const col of tableDiff.addedColumns) {
        statements.push(
          `ALTER TABLE ${tableDiff.table} DROP COLUMN IF EXISTS ${col.name};`
        );
      }
      ...
      for (const col of tableDiff.removedColumns) {
        statements.push(
          `ALTER TABLE ${tableDiff.table} ADD COLUMN ${this.generateColumnSQL(col)};`
        );
      }
