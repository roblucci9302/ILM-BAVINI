...
describe('MigrationGenerator', () => {
  ...
  const createTestTable = (overrides?: Partial<Table>): Table => ({
    ...
    columns: [
      ...
      createTestColumn({ name: 'created_at', type: 'timestamptz', isPrimaryKey: false, isUnique: false, defaultValue: 'now()' }),
    ],
    indexes: [],
    constraints: [],
    ...overrides,
      ...
      const target = createTestSchema([
        createTestTable({
          columns: [
            ...
            createTestColumn({ name: 'bio', type: 'text', isPrimaryKey: false, isUnique: false, nullable: true, defaultValue: undefined }),
          ],
        }),
      ]);
    ...
    it('should generate migration for dropped column', () => {
      const current = createTestSchema([
        createTestTable({
          columns: [
            ...createTestTable().columns,
            createTestColumn({ name: 'temp_column', type: 'text', isPrimaryKey: false, isUnique: false, nullable: true, defaultValue: undefined }),
          ],
        }),
      ]);
      const target = createTestSchema();
