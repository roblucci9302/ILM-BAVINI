...
export class MigrationGenerator {
  ...
  generate(
    ...
    const downStatements = this.options.generateRollback
      ? this.generateDownSQL(diff, currentSchema)
      : [];
  ...
  private columnsEqual(a: Column, b: Column, options?: DiffOptions): boolean {
    ...
    if (a.isForeignKey !== b.isForeignKey) return false;
    if (a.isUnique !== b.isUnique) return false;

    if (!options?.ignoreDefaults && a.defaultValue !== b.defaultValue) return false;
    if (!options?.ignoreComments && a.comment !== b.comment) return false;
  ...
  private policiesEqual(a: RLSPolicy, b: RLSPolicy): boolean {
    ...
    if (a.using !== b.using) return false;
    if (a.check !== b.check) return false;
    if (a.permissive !== b.permissive) return false;
    if (JSON.stringify(a.roles) !== JSON.stringify(b.roles)) return false;
  ...
  private analyzeWarnings(diff: SchemaDiff): string[] {
    ...
    for (const tableDiff of diff.modifiedTables) {
      for (const colDiff of tableDiff.modifiedColumns) {
        ...
        if (before.type !== after.type) {
          warnings.push(
            `⚠️ Changement de type ${tableDiff.table}.${colDiff.name}: ${before.type} → ${after.type}`
          );
        }
      }
    }
