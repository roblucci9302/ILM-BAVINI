...
export class RLSGenerator {
   ...
   */
  generateTablePolicies(
    table: Table,
    config?: TableRLSConfig
  ): { policies: RLSPolicy[]; warnings: string[] } {
    const policies: RLSPolicy[] = [];
    const warnings: string[] = [];
   ...
   */
  private generateTeamBasedPolicies(
    table: Table,
    teamColumn?: string,
    ownerColumn?: string
  ): RLSPolicy[] {
  ...
  private detectOwnerColumn(table: Table): string | undefined {
    ...
    if (this.options.customOwnerColumn) {
      ...
      if (customCol) return customCol.name;
    }
    ...
    const userFk = table.columns.find(
      c => c.isForeignKey && c.references?.table === 'users'
    );
    if (userFk) return userFk.name;
  ...
  private isLookupTable(table: Table): boolean {
    ...
    return lookupPatterns.some(p => tableName.includes(p)) &&
           !this.detectOwnerColumn(table) &&
