...
export class SchemaGenerator {
  ...
  extractEntities(description: string): EntityExtractionResult {
    ...
    const confidence = entities.length > 0
      ? Math.min(100, 70 + entities.filter(e => e.source === 'explicit').length * 5)
      : 0;
  ...
  private extractColumnsForEntity(description: string, entityName: string): ExtractedColumn[] {
    ...
    for (const col of commonColumns) {
      if (!seenColumns.has(col.name)) columns.push(col);
    }
  ...
  private inferCommonColumns(entityName: string): ExtractedColumn[] {
    ...
    if (name.includes('user') || name.includes('utilisateur') || name.includes('membre')) {
      columns.push(
        ...
        { name: 'full_name', inferredType: 'text', isRequired: false, isUnique: false, source: 'inferred', confidence: 85 },
      );
    }
    ...
    if (name.includes('product') || name.includes('produit') || name.includes('article')) {
      columns.push(
        ...
        { name: 'price', inferredType: 'numeric', isRequired: true, isUnique: false, source: 'inferred', confidence: 85 },
        { name: 'description', inferredType: 'text', isRequired: false, isUnique: false, source: 'inferred', confidence: 80 },
      );
    }

    if (name.includes('order') || name.includes('commande')) {
      columns.push(
        { name: 'status', inferredType: 'varchar', isRequired: true, isUnique: false, source: 'inferred', confidence: 90 },
        { name: 'total', inferredType: 'numeric', isRequired: true, isUnique: false, source: 'inferred', confidence: 85 },
      );
    }
    ...
    if (name.includes('post') || name.includes('blog')) {
      columns.push(
        ...
        { name: 'content', inferredType: 'text', isRequired: true, isUnique: false, source: 'inferred', confidence: 85 },
        { name: 'published', inferredType: 'bool', isRequired: false, isUnique: false, source: 'inferred', confidence: 80 },
      );
    }

    if (name.includes('comment') || name.includes('commentaire')) {
      columns.push({ name: 'content', inferredType: 'text', isRequired: true, isUnique: false, source: 'inferred', confidence: 90 });
    }
    ...
    for (const entity of entities) {
      for (const relation of entity.relations) {
        if (relation.type === 'N-N' && relation.throughTable) {
          ...
          if (createdJunctions.has(normalizedName)) continue;
          ...
          junctionTables.push({
            ...
            schema: this.options.defaultSchema,
            columns: [
              { name: fk1, type: 'uuid', nullable: false, isPrimaryKey: true, isForeignKey: true, isUnique: false, references: { table: entity.tableName, column: 'id', onDelete: 'CASCADE' } },
              { name: fk2, type: 'uuid', nullable: false, isPrimaryKey: true, isForeignKey: true, isUnique: false, references: { table: relation.targetEntity, column: 'id', onDelete: 'CASCADE' } },
              { name: 'created_at', type: 'timestamptz', nullable: false, defaultValue: 'now()', isPrimaryKey: false, isForeignKey: false, isUnique: false },
            ],
            indexes: [],
    ...
    for (const table of tables) {
      for (const column of table.columns) {
        if (column.isForeignKey) {
          indexes.push({ name: `idx_${table.name}_${column.name}`, table: table.name, columns: [column.name], isUnique: false, method: 'btree' });
        }
      }

      if (table.columns.some(c => c.name === 'created_at')) {
        indexes.push({ name: `idx_${table.name}_created_at`, table: table.name, columns: ['created_at'], isUnique: false, method: 'btree' });
      }

      for (const column of table.columns) {
        if (column.isUnique && !column.isPrimaryKey) {
          indexes.push({ name: `idx_${table.name}_${column.name}_unique`, table: table.name, columns: [column.name], isUnique: true, method: 'btree' });
        }
      }
    }
  ...
  private generateTableSQL(table: Table): string {
    ...
    if (pkColumns.length === 1) {
      ...
      if (pkIndex !== -1) columnDefs[pkIndex] += ' PRIMARY KEY';
  ...
  private toSnakeCase(str: string): string {
    return str.replace(/([A-Z])/g, '_$1').replace(/[-\s]+/g, '_').replace(/^_/, '').toLowerCase().replace(/_+/g, '_');
  }
  ...
  private singularize(word: string): string {
    ...
    if (word.endsWith('es') && (word.endsWith('ches') || word.endsWith('shes'))) return word.slice(0, -2);
