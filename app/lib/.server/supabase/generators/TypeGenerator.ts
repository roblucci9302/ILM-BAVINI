...
export class TypeGenerator {
  ...
  private generateDatabaseInterface(schema: Schema): string {
    ...
    for (const table of schema.tables) {
      ...
      if (this.options.generateInsertTypes) lines.push(`        Insert: ${pascalName}Insert;`);
      if (this.options.generateUpdateTypes) lines.push(`        Update: ${pascalName}Update;`);
  ...
  private isOptionalForInsert(column: Column): boolean {
    if (column.defaultValue) return true;
    if (column.nullable) return true;
    if (['id', 'created_at', 'updated_at'].includes(column.name)) return true;
    return false;
  }
  ...
  private getZodType(column: Column): string {
    ...
    if (column.name === 'email') return 'z.string().email()';
    if (column.name.includes('url') || column.name.includes('website')) return 'z.string().url()';
