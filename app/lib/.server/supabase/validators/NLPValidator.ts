...
const SQL_RESERVED_WORDS = new Set([
  'all', 'and', 'any', 'array', 'as', 'asc', 'between', 'by', 'case', 'check',
  'column', 'constraint', 'create', 'cross', 'current', 'default', 'delete',
  'desc', 'distinct', 'drop', 'else', 'end', 'exists', 'false', 'for', 'foreign',
  'from', 'full', 'group', 'having', 'in', 'index', 'inner', 'insert', 'into',
  'is', 'join', 'key', 'left', 'like', 'limit', 'not', 'null', 'on', 'or',
  'order', 'outer', 'primary', 'references', 'right', 'select', 'set', 'table',
  'then', 'to', 'true', 'union', 'unique', 'update', 'using', 'values', 'when',
  'where', 'with', 'user', 'role', 'grant', 'revoke',
]);
...
const TYPE_INFERENCE_PATTERNS: Array<{
  ...
  { pattern: /created_at|updated_at|deleted_at|_at$/i, type: 'timestamptz', confidence: 95, reason: 'Timestamp détecté' },
  ...
  { pattern: /^is_|^has_|^can_|^should_|^enabled$|^active$|^visible$/i, type: 'bool', confidence: 95, reason: 'Booléen détecté' },
  ...
  { pattern: /price|cost|amount|total|balance|salary/i, type: 'numeric', confidence: 90, reason: 'Montant monétaire détecté' },
  ...
  { pattern: /metadata|settings|config|options|data|json|payload/i, type: 'jsonb', confidence: 85, reason: 'Données JSON détectées' },
  { pattern: /name|title|label|description|bio|content|text|message|comment/i, type: 'text', confidence: 80, reason: 'Texte détecté' },
  ...
  { pattern: /avatar|image|photo|picture|file|attachment/i, type: 'text', confidence: 80, reason: 'URL de fichier détectée' },
];
...
export class NLPValidator {
  ...
  async validate(
    ...
    const requiresConfirmation =
      overallConfidence < CONFIDENCE_THRESHOLDS.AUTO_ACCEPT ||
      errors.length > 0 ||
      suggestedQuestions.length > 0;

    const isValid =
      errors.filter(e => e.severity === 'critical').length === 0 &&
      overallConfidence >= this.options.minConfidence;
  ...
  private validateEntityName(name: string, tableName: string): {
    errors: ValidationError[];
    warnings: ValidationWarning[];
    penalty: number;
  } {
  ...
  private validateColumn(column: ExtractedColumn, entityName: string): {
    errors: ValidationError[];
    warnings: ValidationWarning[];
    questions: string[];
    penalty: number;
  ...
  private checkNameConflicts(entities: ExtractedEntity[], existingTables?: string[]): {
    errors: ValidationError[];
    warnings: ValidationWarning[];
    questions: string[];
  } {
  ...
  private calculateOverallConfidence(entityConfidences: EntityConfidence[]): number {
    if (entityConfidences.length === 0) return 0;
  ...
  private suggestAlternativeTypes(columnName: string): string[] {
    ...
    return matches.sort((a, b) => b.confidence - a.confidence).slice(0, 3).map(m => m.type);
  }
  ...
  private generateSuggestions(errors: ValidationError[], warnings: ValidationWarning[]): string[] {
    ...
    for (const warning of warnings) {
      if (warning.suggestion) suggestions.push(warning.suggestion);
    }
