...
export class RLSValidator {
  ...
  private isOverlyPermissive(policy: RLSPolicy): boolean {
    const checkTrue = (expr: string | undefined) =>
      expr?.trim().toLowerCase() === 'true';
  ...
  private generateSuggestions(analyses: PolicyAnalysis[]): string[] {
    ...
    const hasPermissive = analyses.some(a =>
      a.issues.some(i => i.includes('permissive'))
    );
    if (hasPermissive) {
      suggestions.push('Éviter les politiques "true" sauf pour les lectures publiques intentionnelles');
    }

    const hasNoAuth = analyses.some(a =>
      a.issues.some(i => i.includes('authentification'))
    );
    if (hasNoAuth) {
