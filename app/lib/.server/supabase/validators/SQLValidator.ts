...
export class SQLValidator {
  ...
  private checkIdentifiers(sql: string): {
    ...
    const identifierRegex = /(?:CREATE\s+TABLE|ALTER\s+TABLE|INSERT\s+INTO|UPDATE|DELETE\s+FROM|FROM)\s+([a-zA-Z_][a-zA-Z0-9_]*)/gi;
    let match;
   ...
   */
  async validateMigration(up: string, down: string): Promise<{
    isValid: boolean;
  ...
  private checkReversibility(up: string, down: string): boolean {
    ...
    for (const column of upOperations.addedColumns) {
      if (!downOperations.droppedColumns.some(
