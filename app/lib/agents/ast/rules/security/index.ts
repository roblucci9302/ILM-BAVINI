  ...
  analyze(node: ts.Node, context: RuleContext): void {
    ...
    if (ts.isCallExpression(node)) {
      ...
      if (ts.isIdentifier(expression) && expression.text === 'eval') {
        context.report(this.createIssue(node, context.sourceFile,
          ...
          {
            suggestion: 'Utilisez JSON.parse() pour les données JSON, ou restructurez votre code pour éviter l\'évaluation dynamique.',
          }
        ));
      }
    }
    ...
    if (ts.isNewExpression(node)) {
      ...
      if (ts.isIdentifier(expression) && expression.text === 'Function') {
        context.report(this.createIssue(node, context.sourceFile,
          'new Function() est similaire à eval() et devrait être évité.',
          {
            severity: 'warning',
...
export class NoInnerHTMLRule extends BaseRule {
  ...
  analyze(node: ts.Node, context: RuleContext): void {
    ...
    if (ts.isBinaryExpression(node) &&
      ...
      if (ts.isPropertyAccessExpression(left) &&
          (left.name.text === 'innerHTML' || left.name.text === 'outerHTML')) {
        context.report(this.createIssue(node, context.sourceFile,
  ...
  protected getExamples(): RuleExample[] {
    return [
      ...
      { description: 'dangerouslySetInnerHTML génère un warning', code: '<div dangerouslySetInnerHTML={{__html: html}} />', isValid: false },
...
export class SQLInjectionRule extends BaseRule {
  ...
  private hasVariableInConcat(node: ts.BinaryExpression): boolean {
    ...
    return isVariable(left) || isVariable(right) ||
  ...
  protected getExamples(): RuleExample[] {
    return [
      { description: 'Template literal SQL avec variable', code: '`SELECT * FROM users WHERE id = ${userId}`', isValid: false },
      { description: 'Concaténation SQL', code: '"SELECT * FROM users WHERE name = \'" + name + "\'"', isValid: false },
      { description: 'Requête paramétrée est OK', code: 'db.query("SELECT * FROM users WHERE id = ?", [userId])', isValid: true },
    ];
  }
}
...
export class XSSPreventionRule extends BaseRule {
  ...
  protected getExamples(): RuleExample[] {
    return [
      ...
      { description: 'postMessage sans vérification origin', code: 'window.addEventListener("message", (e) => handle(e.data))', isValid: false },
      { description: 'postMessage avec vérification origin', code: 'window.addEventListener("message", (e) => { if (e.origin === "https://trusted.com") handle(e.data) })', isValid: true },
    ];
  }
}
