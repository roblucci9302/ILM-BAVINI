...
export interface UseCheckpointsReturn {
  ...
  // Actions
  createCheckpoint: (description?: string, triggerType?: 'manual' | 'auto' | 'before_action') => Promise<Checkpoint | null>;
...
export function useCheckpoints(options: UseCheckpointsOptions): UseCheckpointsReturn {
  ...
  const createCheckpoint = useCallback(async (
    ...
    try {
      ...
      const currentCheckpoints = Object.values(checkpointsMap.get()).filter(
        (cp) => cp.chatId === chatId
      );
