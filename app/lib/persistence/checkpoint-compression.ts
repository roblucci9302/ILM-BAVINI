...
export async function compressSnapshot(
  ...
  const originalSize = calculateStringSize(filesJson) +
  ...
  try {
    // Compress each field separately for better error handling
    const [compressedFiles, compressedMessages, compressedActions] = await Promise.all([
    ...
    const compressedSize = calculateStringSize(compressedFiles) +
