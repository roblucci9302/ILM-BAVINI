...
export async function downloadFile(client: SupabaseClient, bucket: string, path: string): Promise<Blob> {
  const response = await fetch(`${(client as any).url}/storage/v1/object/${bucket}/${path}`, {
    headers: {
      apikey: (client as any).anonKey,
      Authorization: `Bearer ${(client as any).accessToken || (client as any).anonKey}`,
    },
  });
