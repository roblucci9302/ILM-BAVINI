...
describe('checkpoints store integration', () => {
  ...
  beforeAll(async () => {
    ...
    await db.query(
      ...
      ['integration-chat-1', 'url-1', 'Test Chat', '[]'],
    );
  });
  ...
  describe('data integrity', () => {
    it('should maintain consistency between store and database', async () => {
      ...
      expect(JSON.stringify(storeCheckpoint?.filesSnapshot)).toBe(
        JSON.stringify(dbCheckpoint?.filesSnapshot)
      );

      // Messages snapshot should match
      expect(JSON.stringify(storeCheckpoint?.messagesSnapshot)).toBe(
        JSON.stringify(dbCheckpoint?.messagesSnapshot)
      );
    });
