 ...
 */
export const checkpointsMap = map<Record<string, Checkpoint>>(
  import.meta.hot?.data?.checkpointsMap ?? {}
);

/**
 * Current active checkpoint ID (if viewing a restored state).
 */
export const currentCheckpointId = atom<string | null>(
  import.meta.hot?.data?.currentCheckpointId ?? null
);

/**
 * Whether a restore operation is in progress.
 */
export const isRestoring = atom<boolean>(
  import.meta.hot?.data?.isRestoring ?? false
);
...
export const isLoading = atom<boolean>(
  import.meta.hot?.data?.isLoading ?? false
);

/**
 * Last error that occurred during checkpoint operations.
 */
export const checkpointError = atom<Error | null>(
  import.meta.hot?.data?.checkpointError ?? null
);

/**
 * Timestamp of the last auto-checkpoint (for throttling).
 */
export const lastAutoCheckpointTime = atom<number>(
  import.meta.hot?.data?.lastAutoCheckpointTime ?? 0
);

/**
 * Current chat ID for filtering checkpoints.
 */
export const currentChatId = atom<string | null>(
  import.meta.hot?.data?.currentChatId ?? null
);
 ...
 */
export const currentChatCheckpoints = computed(
  [checkpointsList, currentChatId],
  (list, chatId) => {
    if (!chatId) return [];
...
export function shouldCleanupCheckpoints(chatId: string): boolean {
  const config = checkpointConfig.get();
  const chatCheckpoints = Object.values(checkpointsMap.get()).filter(
    (cp) => cp.chatId === chatId
  );
