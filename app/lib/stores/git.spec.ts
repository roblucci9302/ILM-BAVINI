...
describe('git store', () => {
  ...
  describe('clearGitError', () => {
    it('should clear error in store', () => {
      setGitError('Some error');
      clearGitError();
  ...
  describe('resetGitStore', () => {
    it('should reset all values to initial state', () => {
      // Set various values
      setRepoInfo('/path', 'https://example.com');
      setBranches('main', ['main', 'develop']);
      setChanges([{ path: 'test.ts', status: 'modified' }]);
      setGitStatus('pushing', 'Pushing...');
      setGitError('Some error');
      setHasAuth(true);
